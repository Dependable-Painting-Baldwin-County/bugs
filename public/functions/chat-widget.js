...
(function(){
  ...
  const form = h('form',{class:'dp-estimate-form',onsubmit:async (/** @type {SubmitEvent} */ e)=>{
      ...
      const payload = {
        ...
        message: msgEl?.value.trim()||'Chat quick estimate',
        source:'chat_widget',
        page: location.pathname,
        session: sid()
      };
      try {
        const resp = await fetch('/api/estimate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
        if(resp.ok){ form.innerHTML='<p style="font-size:12px;color:#8ef;">Estimate request received. We\'ll reach out shortly.</p>'; }
        ...
        if(bodyStream){
          const reader=bodyStream.getReader();
            ...
            while(true){
              ...
              const last=parts.pop(); buffer=last||'';
              for(const part of parts){
                if(!part.trim()) continue;
                if(part.includes('[DONE]')) continue;
                const line=part.split(/data:/).pop()||'';
                try { const chunk=JSON.parse(line); full+=(full?' ':'')+chunk; aiEl.textContent=full; } catch{}
              }
            }
        } else { aiEl.textContent='[Stream unavailable]'; }
        try {
          const metaResp=await fetch(ENDPOINT,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:'[context]',session:sVal,page:location.pathname})});
          if (!metaResp.ok) throw new Error('Metadata fetch failed');
          const m=await metaResp.json().catch(err => {
            console.warn('Failed to parse metadata JSON:', err);
            return { intents: null };
          });
          aiEl.appendChild(buildLinks(full));
          aiEl.appendChild(addFeedback(sVal,full));
          const est=maybeEstimateForm(m.intents);
          if(est) bodyEl.appendChild(est);
        } catch (err) {
          console.warn('Metadata fetch error:', err);
        }
      } else {
        const data=await r.json().catch(err => {
          console.error('Failed to parse chat response JSON:', err);
          return { reply: 'Error parsing response. Please try again.' };
        });
        tEl.remove();
        sVal=data.session||sVal;
        const answer=data.reply||data.answer||'[No answer]';
        const aiEl=addMsg(answer,'ai');
        aiEl.appendChild(buildLinks(answer));
        aiEl.appendChild(addFeedback(sVal,answer));
        const est=maybeEstimateForm(data.intents);
        if(est) bodyEl.appendChild(est);
      }
    } catch {
      tEl.remove();
      addMsg('Error fetching answer. Please call '+PHONE_DISPLAY+'.','ai');
    }
